...
# Log class to keep proper logs that can be utitlised by graph plotter or visualiser
class Log:
    def __init__(self, eveTime, eveName, stn, serv, stncount, servcount):
        self.eveName = eveName
        self.eveTime = eveTime
        self.stn = stn
        self.serv = serv
        self.stncount = stncount
...
class Service:
    def __init__(self, servNum):
        ...
        self.stnTimingsDict = {}
        self.buckets = {}
        # total crowd the service is carrying 
        self.total = 0  
...
with open(supServices, newline='') as file:
    ...
    servNums = header[4:]  # first four rows/columns are not yet the main data
    ...
    if ev.etype == 'servArrival':  # do some bucket transfer only for those stations with event time
        ...
        for stn in stnList:  # at the stn corresponding to event, update for *EACH* station to/from service
            if servicesDict[ev.servNum].stnTimingsDict[stn] > ev.timing:
                ...
                print('not boarded', ev.stn, ev.timing)
        # adding servArrival to the log
        LogList.append(Log( ev.timing,"Service Arrival", ev.stn, ev.servNum, sum(stnObjDict[ev.stn].buckets.values()), sum(servicesDict[ev.servNum].buckets.values())))
        ...
        # adding passenger Vanish to the log
        LogList.append(Log(ev.timing, f"Passenger Vanish -{ev.bucket}", ev.stn, '-', sum(stnObjDict[ev.stn].buckets.values()), '-'))

    elif ev.etype == 'passAppear':
        print('came into passAppear')
        stnObjDict[ev.stn].buckets[ev.destn] += ev.bucket
        # adding passenger appear to the log
        LogList.append(Log(ev.timing, f"Passenger Appear +{ev.bucket}", ev.stn, '-', sum(stnObjDict[ev.stn].buckets.values()), '-'))
...
class Station:
    def __init__(self, stnName):
        ...
        # total crowd the station is containing
        self.total = 0
...
for serv in servicesLst:
    ...
    for timing in serv.stnTimings:
        ...
        EventsList.append(Event(timing + 5, stn, serv.servNum, 'passVanish', 0, 0, None))
...
with open("log/event_log.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Timestamp", "Event Type", "Station", "Service", "Current Station crowd", "Current service Crowd"])
    for item in LogList:
        writer.writerow([item.eveTime, item.eveName, item.stn, item.serv, item.stncount, item.servcount])
